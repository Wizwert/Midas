...
namespace Midas.ObjectModel
{
    ...
    {
        private static readonly IEnumerable<PropertyInfo> MemberData = typeof(SecurityMarketData).GetProperties(BindingFlags.Public | BindingFlags.Instance);

        public SecurityMarketData(XElement element)
        {
            foreach(var memberInfo in MemberData)
            {
                var test = element.Descendants(memberInfo.Name).Single();

                memberInfo.SetValue(this, test.Value);
            }
        }
